...
## 2026-08-08 晚間（回饋訊號 UX 第二梯＋講者混卡豁免梯落地後）

- [x] **回饋訊號 UX 第二梯** ✅（`205a5bf`、`1c3b2de`）：toast 系統取代 29 處 alert、
  轉錄徽章基準修正、dashboard 單資料夾壞檔容錯。獨立驗收通過。
- [x] **round-trip 守門測試＋repo 開發約定落地** ✅（`60f1597` 測試、`6e8c4d6` CLAUDE.md、
  `087c395` 計畫檔）。
- [x] **講者混卡豁免梯（C 梯）** ✅（`64a24ef` 修正＋13 條回歸測試、`56ab015` 計畫檔）：
  smooth_speakers 加反應詞豁免（26 詞白名單＋語氣詞結尾 ≤8 字）＋豁免段透明化
  （越 ≤4 字豁免牆取真鄰居、遇同講者透明段即停、候選 ≥4 字才跳過）。
  魁哥整集三輪獨立驗收：傷害改標 span 45→30、真歸錯人混卡 32→25、講者切換 135→178、
  909 passed。碎卡 73→90 經逐張分解證明與「傷害 ≤30」在此資料上結構性互斥（下限 84），
  門檻重定 ≤92（由獨立驗收者核實證明後放行）。
  ⚠ 傷害 30/30、真混卡 25/25 **貼線餘裕 0** —— 下一集實測要重量這兩個數字，超線回頭調豁免詞表。
  殘餘議題（#71 豁免被段合併繞過、#143 定義性犧牲）在
...
- [x] 字幕潤飾：加入閃字幕合併、超長卡持續切分、逐字時間切點、輸出驗證與時間戳備份。
- [x] 儲存隔離：`/api/save` 驗證 `episode_dir`，前端共用存檔通道補上 HTTP 409 復原。
- [x] 背景工作隔離：轉錄狀態加入 `job_id` 與 `episode_dir`，前端忽略其他集數狀態。
...
- [ ] 統一 `pyproject.toml` 與 `setup_app.py` 版本來源，更新 README 安裝流程。
