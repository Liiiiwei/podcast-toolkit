// 字幕時間軸（循環 import：timeline.js 也會匯入本檔的共用符號，詳見該檔頭註解的 TDZ 規則）
import {
  TL_ZOOM_MIN,
  TL_ZOOM_MAX,
  TL_ZOOM_STEP,
...
export function rerenderEditState() {
  renderTopbar();
  renderCards();
  renderCaption();
  renderTypo();
...
export function fmtTimeCard(sec) {
...
export function fmtTime(sec, ref = sec) {
...
async function loadEpisodeState() {
  ...
  // 傳輸與 409（尚未選集 → 導回 dashboard）在 api.js；這裡只負責把欄位灌進 state。
  const data = await apiGetEpisode();
...
export let _lastActiveKey = null;
...
// === 儲存 / 取消 ===
// postSave 已搬進 api.js（序列化通道 + 集數儲存隔離的 409 重開）；此處僅 import 使用。
