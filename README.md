...
把 podcast 的**原始錄影 + 錄音**，一路做成可以上架的**成品影片（YT 完整版）和音檔（MP3）**的工具。
...
flowchart TD
    ...
    D0 -- "否" --> E1["設定畫面<br/>鏡頭 · 裁切 · 頭尾trim · 字級"]
    D1 --> E1

    E1 --> F0["選字幕模式<br/>燒進畫面 / 另存 .srt / 抽換"]
    F0 --> S1["按『輸出』彈出合成設定<br/>⏱ 倍速 · 去空拍 · 節目封面"]
    S1 --> F1{"要出哪個版本？"}
...
> 一句話：把「錄完的雙機素材」變成「可上架的 YT／MP3」，中間該做的事大多自動化，且**全程本地不上雲、免 API 金鑰**。

**轉字幕與校對**
- **一鍵 Breeze 轉字幕（本地、自動標講者）** — 按一顆就把分軌錄音整條龍跑完：各軌 mic 聽打 → 自動標誰在講（a/b/c）→ 匯入字幕卡 → 本地校對。免金鑰、聲音不上雲。
- **AI 逐句字幕校對（本地零金鑰）** — 轉完自動用 App 內建的 Qwen3 4B 與 llama.cpp，依保守規則修同音近音錯字、台灣慣用字與代名詞；只改文字不動時間，完全離線且不需安裝 Claude Code。
...
**出片設定**
- **裁切／頭尾 trim** — 影片下方一個面板搞定重新構圖、切掉開場閒聊與收尾空白；時間軸把手拖曳時會浮出精確時間（m:ss.d），對切點不用猜。
- **倍速 + 全片去空拍** — 只加速正片（片頭尾不動、字幕自動同步），並自動跳剪中段長停頓，成品更緊湊、資訊密度更高。

**輸出**
- **一鍵輸出** — 同一集產出 16:9 含片頭尾的 YT 完整版.mp4 與純音訊原速 MP3，各自套好裁切與字幕樣式。
...
| **倍速** | 只加速「正片」（片頭片尾不動），預設 1.1 倍，影片更精簡。 |
...
`install.sh` 會安裝開發用 CLI 與 Breeze 環境；大型模型不放進 Git。若要在另一台 Apple Silicon Mac 建立完整 App，需另外準備 Breeze 與本機語言模型 sidecar，再執行：

```bash
export BREEZE_STAGE="$HOME/.podcast-toolkit/breeze-stage"
export LOCAL_LLM_STAGE="$HOME/.podcast-toolkit/local-llm-stage"
...
建置前檢查會確認 `breeze-asr-25`、Qwen3 4B、llama.cpp 的必要執行檔、套件目錄、詞典、授權與模型雜湊都正確。完整 App 內含兩套模型，執行轉字幕、字幕校對與上架文案時都不需網路或 API 金鑰。
...
- **⚠ 待複查（黃色 ⚠ 在時間旁）** — 這句**內容**可能怪怪的，請看一眼別急著刪：
  ...
  - 上方工具列可「**只看待複查**」篩選，或用 **上一張** ／ **下一張**（或鍵盤 J／K）一張接一張審；看完一張按卡上的 **看過**，它會淡掉、從待複查計數扣掉、J／K 也跳過它（只存這次編輯、不寫原檔，換集就清空）。
...
打開轉字幕視窗時，上方會先列出**它認出了哪幾軌音檔**（軌別由檔名自動判斷，檔名含 `Track1`／`Mic 2` 才會認成分軌），不用自己猜：

- 「讀取音檔清單中…」— 還在掃。
- 「**已辨識到音檔：分軌 3 軌、混音 1 個**」— 找到了但還沒指定角色。
- 「**已設定分軌：3 軌**」— 已經配好對應，字幕會標講者。
- 「**沒有偵測到外接音檔**」— 會直接用影片本身的聲音轉錄；補放 wav 進集資料夾或 `01_母帶`／`02_素材` 後重開這個視窗。

如果是雙機 + 分軌、想要自動切鏡頭：

1. 轉字幕視窗「進階：分軌轉錄」→「設定並啟用分軌轉錄」（已設定過的話按鈕會變成「重新設定分軌」）。
2. 每軌指定對應的音檔 + 標 **主持 / 來賓**，並設「來賓講滿幾秒切特寫」（預設 15 秒）。
3. 存檔 → 自動生成鏡頭規則（`camera_rule`）：**主持→全景 cam A、來賓→特寫 cam B**。來賓軌號每集不同也沒關係，由你標的角色決定。

> 真的按下分軌轉錄時，會另外跳一個視窗讓你勾要轉哪幾軌：每軌標著**已轉過 / 未轉**，已轉過的**預設不勾**避免誤覆寫（也有「只選未轉」一鍵）；硬要重轉的話舊 SRT 會自動備份成時間戳檔。

> 預設仍是**用混音檔轉錄**（比較穩定）。分軌轉錄雖然能標出每張卡的講者，但三支麥克風之間容易串音，可能把鄰座講的話辨識成自己。

### 第 4 步：設定畫面（鏡頭/裁切/頭尾/字級/倍速/去空拍）

分兩處：頂部「**鏡頭**」按鈕（點開「鏡頭與音檔對齊」視窗）＋ 影片下方的「**出片設定**」摺疊面板（預設收合，寫著「裁切比例 · 頭尾 · 字級」，點一下展開）。
...
- **字幕字級**：`字幕 − [值] ＋`，即時預覽（預覽字級對齊實際燒字幕輸出）。
- **頭尾 trim**：設頭／設尾／從頭播／重設，或拖 seek bar 兩端把手，切掉開頭試音、結尾收場；也有「智慧建議」自動抓頭尾靜音。
  - 拖曳／鍵盤微調時把手上方會浮出**精確時間（m:ss.d）**，滑過進度條也會顯示目標時間，不用靠眼睛估。
  - 鍵盤微調：**←→** 一小步、**Shift +←→** 0.5 秒、**Cmd +←→** 1 秒。

> **倍速 / 去空拍 / 節目封面**不在這個面板，而是按下輸出時跳出的「**合成設定**」視窗裡開關（倍速預設 1.1、去空拍偵測 ≥0.8 秒的停頓跳剪、節目封面壓在正片右上角）。
> **旋轉拉正**已從介面移除（實測拖慢輸出、極少用到）；後端仍支援，真的錄歪要手動在 `episode.yaml` 寫 `rotate`。

### 第 5 步：輸出

頂部右側「**輸出 ▾**」下拉（在「鏡頭」按鈕右邊）。展開後上半先選**字幕模式**、下半是三個輸出動作：
...
| `crop_yt` | 裁切框 `{x,y,width,height}`（0–1 比例） |
...
| 階段 | 詞庫怎麼作用 | 吃詞庫的哪部分 |
|---|---|---|
| ① **轉字幕當下**（一鍵 Breeze） | `canonical` 被組成提詞餵給引擎，提高把專名聽對的機率 | `canonical` |
...
- `podcast proofread <集>` — 使用內建 Qwen3 4B 本地校對，完全離線、不需 Claude Code
...
- `podcast publish-doc <集> [--ep N]` — 使用內建 Qwen3 4B 產 YouTube 上架文案：標題／說明欄／章節／重點／社群短文（章節時間對齊成品時間軸，完全離線；介面上沒有這個功能）

Exit codes：0 成功、1 輸出已存在、3 檔案缺失、4 ffmpeg 失敗。

### `defaults.yaml`（全域預設，改了影響所有集）

主要區段：`subtitle_style`（字幕樣式）、`camera_rule`（鏡頭規則預設）、`silence_trim`（去空拍預設）、`speed`（倍速）、`watermark`（封面浮水印）、`assets`（片頭片尾路徑）、`encode`（編碼參數：codec / crf / resolution `1920x1080` / framerate `30000/1001`）。
