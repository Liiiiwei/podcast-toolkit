...
## 附錄第 3 項：`nextKeepTime` 保留卡守衛裁決（2026-09-25 完成）

**開工第一問「幾個地方在管」= 2**：同一個行為（預覽播到被剪掉的段落該不該跳過）
由兩套機制在管 —— `padAndMergeCuts` 的夾制（前後端同一套、有差分測試逐位元比對）
與 `nextKeepTime` 裡的保留卡守衛（只有前端有）。依專案 CLAUDE.md，答案 ≥2 要先併軌。

**為什麼不是「反正 0 紅就刪」**：夾制上線後「整個守衛拿掉」這個突變 0 紅，
但那只證明摸不到。先找出它**唯一還摸得到**的形狀再裁決：夾制只讓位給卡界落在
區間內的保留卡，保留卡**整個包住**被刪卡時兩個條件都不成立 → 不夾，剪除區間原封不動
躺在保留卡語音裡（逐字時間戳把長卡起點往前推時會出現）。

**裁決理由**：在那個形狀裡合成端沒有對應守衛、照剪，守衛卻讓預覽照播 ——
守衛不是第二層保險，而是**預覽說謊的來源**。一個行為只留一個地方管：夾制是正典，
`nextKeepTime` 只做區間查表。

**證據**：`scripts/cdp-walkthrough/timeline-baseline/verify_nextkeeptime_verdict.py` 13/13
（N1 後端剪除區間 `[[7.35, 8.25]]`、N2 完全落在保留卡 #5 `[7.20, 10.15]` 內、
N3/N4 預覽跳到區間末端、N5 區間外不跳）；突變 MUT-G（把舊守衛原樣貼回 → N3
停在 9.303 變紅，還原後回 9.750）；回歸 `verify_b3_guard_and_shift.py` 27/27
（A5a 要跳、A5d／B4b 不跳三條由夾制獨力守住）；
`tests/test_cut_merge_frontend_parity.py` 5 passed；全測試 1053 passed, 1 xfailed。
